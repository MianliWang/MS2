...
try:
    ...
    from ms2.diagnostics import classify_ms1_reference, classify_ms2_diagnostic  # type: ignore
    from ms2.similarity import (  # type: ignore
        ChiralPairThresholds,
        SpectrumSimilarity,
        align_fragment_spectra,
        parse_fragment_string,
...
def annotate_rows(rows: list[dict[str, str]], thresholds: ChiralPairThresholds | None = None) -> list[dict]:
    ...
    for row in annotated:
        if row.get("peak_a_MS2", "").strip() and row.get("peak_b_MS2", "").strip():
            key = (str(row.get("Peak1", "")), str(row.get("Peak2", "")), str(row.get("dia_window_center", "")))
    ...
    for group in shared_groups.values():
        ...
        for row in group:
            ...
            if PRIORITY_RANK.get(row["ms2_review_priority"], 99) > PRIORITY_RANK["P2_boundary_or_interference"]:
...
def build_report(
    ...
    *,
    ...
    review_rows = [r for r in rows if r.get("peak_a_MS2", "").strip() or r.get("peak_b_MS2", "").strip()]
    review_rows.sort(key=lambda r: (PRIORITY_RANK.get(r["ms2_review_priority"], 99), str(r.get("Compound_ID", ""))))
    if max_spectra > 0:
    ...
    shell = shell.replace("{{SOURCE_AND_DATE}}", f"{html.escape(input_path.name)} · {datetime.now().date().isoformat()}")
    ...
    if runtime_helper is not None:
        ...
        report_path.write_text(shell.replace("<!-- DATA_ANALYTICS_HTML_REPORT_RUNTIME -->", ""), encoding="utf-8")
    gallery_content = (
    ...
    gallery = gallery.replace("{{SOURCE_AND_DATE}}", f"{html.escape(input_path.name)} · {datetime.now().date().isoformat()}")
    gallery = gallery.replace("{{REPORT_CONTENT}}", gallery_content)
    gallery_path.write_text(gallery.replace("<!-- DATA_ANALYTICS_HTML_REPORT_RUNTIME -->", ""), encoding="utf-8")
    for priority in PRIORITY_RANK:
        ...
        priority_gallery = priority_gallery.replace("{{SOURCE_AND_DATE}}", html.escape(input_path.name))
        ...
        (output_dir / f"ms2_spectrum_gallery_{priority}.html").write_text(
            priority_gallery.replace("<!-- DATA_ANALYTICS_HTML_REPORT_RUNTIME -->", ""), encoding="utf-8"
        )
    return {"report": report_path, "gallery": gallery_path, "queue": queue_path, "summary": summary_path, "shell": shell_path, "payload": payload_path}
...
def _similarity_from_row(row: dict[str, str]) -> SpectrumSimilarity | None:
    ...
    return SpectrumSimilarity(
        ...
        int(_number(row.get("peak_a_fragment_count")) or len(parse_fragment_string(row.get("peak_a_MS2")))),
        int(_number(row.get("peak_b_fragment_count")) or len(parse_fragment_string(row.get("peak_b_MS2")))),
...
def _write_queue(path: Path, rows: list[dict]) -> None:
    core = [
        "review_row", "Compound_ID", "MZ", "Peak1", "Peak2",
        "ms1_reference_status", "ms2_diagnostic_status", "ms2_review_priority", "ms2_issue_codes",
        "ms2_cosine", "ms2_entropy_similarity", "ms2_matched_peaks",
        "peak_a_explained_intensity", "peak_b_explained_intensity",
        "peak_a_fragment_count", "peak_b_fragment_count", "dia_window_match_count",
        "peak_a_MS2", "peak_b_MS2", "manual_ms2_label", "manual_issue_codes",
        "manual_notes", "manual_reviewer", "manual_review_date",
    ]
...
def _summary(rows: list[dict], review_rows: list[dict], source: Path) -> dict:
    ...
    both = sum(bool(r.get("peak_a_MS2", "").strip()) and bool(r.get("peak_b_MS2", "").strip()) for r in rows)
    one = sum(bool(r.get("peak_a_MS2", "").strip()) ^ bool(r.get("peak_b_MS2", "").strip()) for r in rows)
    return {
...
def _report_content(rows: list[dict], review_rows: list[dict], source: Path) -> tuple[str, dict]:
    ...
    two_spectra = sum(bool(r.get("peak_a_MS2", "").strip()) and bool(r.get("peak_b_MS2", "").strip()) for r in rows)
    one_spectrum = sum(bool(r.get("peak_a_MS2", "").strip()) ^ bool(r.get("peak_b_MS2", "").strip()) for r in rows)
    ...
    spectra = "".join(_spectrum_card(r, source_name, i <= 8) for i, r in enumerate(review_rows, start=1))
    ...
    payload = {
        "charts": [
            ...
            {
                "id": "similarity-map", "height": 340, "type": "scatter",
                "dataset": {"id": "similarity-map", "title": "Cosine–entropy", "data": scatter_data,
                    "chart_spec": {"id": "similarity-map", "dataset": "similarity-map", "title": "Cosine–entropy", "type": "scatter",
                        "encodings": {"x": {"field": "cosine", "label": "Cosine", "type": "quantitative"}, "y": {"field": "entropy", "label": "Entropy similarity", "type": "quantitative"}, "color": {"field": "status", "type": "nominal"}, "tooltip": [{"field": "compound", "type": "nominal"}]},
                        "xAxisTitle": "Cosine", "yAxisTitle": "Entropy similarity", "valueFormat": "decimal"}}
            },
        ]
    }
...
def _bar_payload(chart_id: str, title: str, data: list[dict], category: str, value: str) -> dict:
    return {"id": chart_id, "height": 320, "type": "bar", "dataset": {"id": chart_id, "title": title, "data": data, "chart_spec": {"id": chart_id, "dataset": chart_id, "title": title, "type": "bar", "encodings": {"x": {"field": category, "type": "nominal"}, "y": {"field": value, "label": "记录数", "type": "quantitative"}}, "settings": {"orientation": "horizontal", "groupMode": "grouped"}, "xAxisTitle": "", "yAxisTitle": "记录数", "valueFormat": "number"}}}
...
def _bar_svg(data: list[dict], category: str, value: str, color: str) -> str:
    ...
    for i, row in enumerate(data):
        ...
        marks.append(f'<text x="{left-12}" y="{y+17}" text-anchor="end" font-size="13" fill="#465166">{html.escape(str(row[category]))}</text>')
        marks.append(f'<rect x="{left}" y="{y+3}" width="{bar:.1f}" height="22" rx="4" fill="{color}" opacity=".88"/>')
        marks.append(f'<text x="{left+bar+8:.1f}" y="{y+19}" font-size="13" font-weight="700" fill="#172033">{row[value]}</text>')
...
def _scatter_svg(data: list[dict]) -> str:
    ...
    for tick in (0, .25, .5, .75, 1):
        ...
        marks.append(f'<line x1="{x}" y1="{top}" x2="{x}" y2="{top+ph}" stroke="#e8ebf0"/><text x="{x}" y="{height-25}" text-anchor="middle" font-size="12" fill="#657085">{tick:g}</text>')
        marks.append(f'<line x1="{left}" y1="{y}" x2="{left+pw}" y2="{y}" stroke="#e8ebf0"/><text x="{left-10}" y="{y+4}" text-anchor="end" font-size="12" fill="#657085">{tick:g}</text>')
    threshold_x = left + .7*pw
    marks.append(f'<line x1="{threshold_x}" y1="{top}" x2="{threshold_x}" y2="{top+ph}" stroke="#bd3d45" stroke-dasharray="6 5"/><text x="{threshold_x+6}" y="{top+14}" font-size="12" fill="#bd3d45">cosine 0.7</text>')
    colors = {"支持同一化合物":"#25845b", "谱图冲突":"#bd3d45", "证据不足":"#d97706"}
    for row in data:
        x, y = left + row["cosine"]*pw, top + (1-row["entropy"])*ph
        marks.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="5" fill="{colors.get(row["status"], "#8790a0")}" opacity=".78"><title>{html.escape(row["compound"])}: cosine={row["cosine"]:.3f}, entropy={row["entropy"]:.3f}</title></circle>')
    marks.append(f'<text x="{left+pw/2}" y="{height-5}" text-anchor="middle" font-size="13" fill="#465166">Cosine</text><text transform="translate(18 {top+ph/2}) rotate(-90)" text-anchor="middle" font-size="13" fill="#465166">Entropy similarity</text>')
...
def _mirror_spectrum_svg(a_text: str, b_text: str, tolerance: float = 0.01) -> str:
    ...
    if max_mz-min_mz < 1: max_mz = min_mz+1
    width, height, left, right, mid = 900, 240, 55, 20, 120
    pw, amp = width-left-right, 88
    scale_x = lambda mz: left + (mz-min_mz)/(max_mz-min_mz)*pw
    ...
    for i,(mz,intensity) in enumerate(a):
        ...
        marks.append(f'<line x1="{x:.2f}" y1="{mid}" x2="{x:.2f}" y2="{y:.2f}" stroke="{"#246bce" if i in ma else "#aeb7c5"}" stroke-width="{2 if i in ma else 1}"/>')
    for j,(mz,intensity) in enumerate(b):
        x, y = scale_x(mz), mid+intensity/max_b*amp
        marks.append(f'<line x1="{x:.2f}" y1="{mid}" x2="{x:.2f}" y2="{y:.2f}" stroke="{"#d97706" if j in mb else "#aeb7c5"}" stroke-width="{2 if j in mb else 1}"/>')
    for tick in range(5):
        mz=min_mz+(max_mz-min_mz)*tick/4; x=scale_x(mz)
        marks.append(f'<text x="{x:.1f}" y="{height-5}" text-anchor="middle" font-size="10" fill="#697487">{mz:.1f}</text>')
    marks.append('<text x="8" y="32" font-size="11" fill="#246bce">Peak 1</text><text x="8" y="212" font-size="11" fill="#d97706">Peak 2</text>')
...
def main(argv=None) -> None:
    ...
    parser.add_argument("--template", type=Path, default=root / "templates" / "ms2_review_report_shell.html")
    parser.add_argument("--runtime-helper", type=Path, help="Optional embed_html_report_runtime.py path.")
    parser.add_argument("--max-spectra", type=int, default=0, help="0 includes every row having at least one spectrum.")
    args = parser.parse_args(argv)
    outputs = build_report(args.input, args.output_dir, template_path=args.template, runtime_helper=args.runtime_helper, max_spectra=args.max_spectra)
