...
def render_ms2_review_png(*, row: dict, mirror: MirrorSpectrumData, metadata: dict, scale: float = 2.0) -> Image.Image:
    ...
    put = lambda x, y, value, size=12, color="#172033", mono=False, bold=False: draw.text(
        (s(x), s(y)), str(value), fill=color, font=font(size, mono, bold)
    )
    ...
    put(42, 60, f"precursor m/z {_fmt(row.get('MZ'), 6)} · supplied RT {_fmt(row.get('Peak1'), 3)} / {_fmt(row.get('Peak2'), 3)} min · separation {_fmt(row.get('rt_separation_sec'), 1)} s", 14, "#4f5c70", mono=True)
    put(42, 88, f"source: {metadata.get('source_context', '')} · pool {metadata.get('source_pool_id') or '—'} · well {metadata.get('source_pooled_well') or '—'}", 11, "#4f5c70")
    put(42, 118, f"MS2 status: {status} · {priority} · availability: {mirror.availability}", 15, bold=True)
    put(42, 146, "issues: " + _clip(issues + _comparison_label(row), 155), 10, "#8a4b08", mono=True)
    put(42, 170, _clip(f"analysis: standard {metadata.get('standard_id', '')} · filter ≥{_fmt(float(metadata.get('min_relative_intensity', 0))*100, 1)}% base peak · match ±{_fmt(metadata.get('fragment_mz_tol'), 3)} {metadata.get('fragment_mz_tol_unit', '')} · {_correlation_label(metadata)}", 185), 8, "#667085", mono=True)
    put(42, 190, _clip(_acquisition_label(metadata.get("acquisition_context") or {}), 185), 8, "#4f5c70", mono=True)
    put(42, 208, _clip(_method_profile_label(metadata.get("method_profile") or {}, metadata.get("acquisition_reconciliation") or {}), 185), 8, "#8a4b08", mono=True)
    _meter(draw, s, font, 55, 242, 210, "cosine", row.get("ms2_cosine"), threshold=metadata.get("min_cosine"))
    _meter(draw, s, font, 285, 242, 210, "entropy (reported)", row.get("ms2_entropy_similarity"))
    _meter(draw, s, font, 515, 242, 210, "explained Peak 1", row.get("peak_a_explained_intensity"), threshold=metadata.get("min_explained_intensity"))
    _meter(draw, s, font, 745, 242, 210, "explained Peak 2", row.get("peak_b_explained_intensity"), threshold=metadata.get("min_explained_intensity"))
    _meter(draw, s, font, 975, 242, 210, "matched fragments", row.get("ms2_matched_peaks"), threshold=metadata.get("min_matched_peaks"), count=True)
    ...
    put(55, 997, f"DIA center {_fmt(row.get('dia_window_center'), 3)} · bounds [{_fmt(row.get('dia_window_lower'), 3)}, {_fmt(row.get('dia_window_upper'), 3)}] · matching windows {_fmt(row.get('dia_window_match_count'), 0)} · config {metadata.get('config_hash', '')}", 10, "#667085", mono=True)
    return image
...
def _meter(draw, s, font, x, y, width, label, value, *, threshold=None, count=False):
    ...
    draw.rounded_rectangle((s(x), s(y + 21), s(x + width), s(y + 47)), radius=s(4), fill="#eef1f5", outline="#dfe4eb")
    if fraction > 0:
        draw.rounded_rectangle((s(x), s(y + 21), s(x + width * fraction), s(y + 47)), radius=s(4), fill="#246bce")
...
def _mirror(draw, s, font, mirror, issues):
    ...
    sx = lambda mz: left + (mz - mirror.min_mz) / (mirror.max_mz - mirror.min_mz) * width
    draw.text((s(70), s(300)), "Normalized mirror spectrum · each side scaled to its own base peak", fill="#25334a", font=font(14, bold=True))
    ...
    for index, (mz, intensity) in enumerate(mirror.peak_a):
        ...
        if matched:
            draw.ellipse((s(x) - s(2.8), s(y) - s(2.8), s(x) + s(2.8), s(y) + s(2.8)), fill="#246bce")
    for index, (mz, intensity) in enumerate(mirror.peak_b):
        ...
        draw.line((s(x), s(mid), s(x), s(y)), fill=color, width=max(1, line_width))
        if matched:
            draw.polygon(((s(x), s(y + 3.5)), (s(x - 3.5), s(y - 2.5)), (s(x + 3.5), s(y - 2.5))), fill="#d97706")
    for tick in range(7):
    ...
    _right_text(draw, s, font, right, 338, "matched: thick + circle/triangle · unmatched: thin grey", 10, "#667085")


def _peak_card(draw, s, font, x, y, title, prefix, row, color):
    draw.rounded_rectangle((s(x), s(y), s(x + 550), s(y + 190)), radius=s(9), fill="#fbfcfe", outline="#dfe4eb")
    ...
    for index, line in enumerate(lines):
        draw.text((s(x + 18), s(y + 48 + index * 25)), line, fill="#4f5c70", font=font(10, mono=True))


def _dia_coverage(draw, s, font, target_mz, windows):
    if not windows:
        draw.text((s(70), s(732)), "Acquired DIA-window metadata unavailable", fill="#8790a0", font=font(11))
        return
    ...
    _right_text(draw, s, font, 1190, 717, f"{min(lowers):.1f}–{max(uppers):.1f} m/z · n={len(windows)}", 10, "#667085")
    draw.rectangle((s(70), s(741), s(1190), s(759)), fill="#f3f5f8", outline="#dfe4eb")
    for index, (lower, upper) in enumerate(zip(lowers, uppers)):
        draw.rectangle((s(sx(lower)), s(741), s(sx(upper)), s(759)), fill="#dce9fb" if index % 2 == 0 else "#eef3fb", outline="#8aaee3")
    if target is not None:
        ...
        draw.text((s(min(max(x + 5, 75), 1040)), s(765)), f"target {target:.4f} · {label}", fill="#8a4b08", font=font(10, mono=True))
...
def _dashed_line(draw, s, x1, y1, x2, y2, color):
    ...
    for index in range(0, segments, 2):
        ...
        draw.line((s(x1 + (x2-x1)*a), s(y1 + (y2-y1)*a), s(x1 + (x2-x1)*b), s(y1 + (y2-y1)*b)), fill=color)
