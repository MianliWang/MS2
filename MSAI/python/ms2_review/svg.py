...
def render_ms2_review_svg(*, row: dict, mirror: MirrorSpectrumData, metadata: dict) -> str:
    """渲染一张完整、可缩放且可独立打开的MS2审核SVG。

    图中同时展示目标/来源、诊断与问题代码、相似度指标、镜像fragment谱、
    DIA覆盖和两侧提取质量。两侧各按自身base peak归一化仅用于显示，CSV中的
    explained intensity和cosine仍使用原始强度。
    """
...
def _mirror_svg(mirror: MirrorSpectrumData, issues: str) -> str:
    """绘制Peak1向上、Peak2向下的归一化镜像fragment谱。"""
    ...
    def scale_x(mz):
        ...
        return left + (mz - mirror.min_mz) / (mirror.max_mz - mirror.min_mz) * plot_width
    ...
    if not mirror.peak_a and not mirror.peak_b:
        marks.append(
            f'<text x="630" y="495" text-anchor="middle" font-size="17" fill="#8790a0">No retained fragment spectrum</text><text x="630" y="525" text-anchor="middle" font-size="11" fill="#8790a0">{html.escape(issues)}</text>'
        )
    for index, (mz, intensity) in enumerate(mirror.peak_a):
        ...
        marks.append(
            f'<line x1="{x:.2f}" y1="{mid}" x2="{x:.2f}" y2="{y:.2f}" stroke="{color}" stroke-width="{stroke}"><title>Peak 1 m/z {mz:.5f}; intensity {intensity:.6g}; {"matched" if matched else "unmatched"}</title></line>'
        )
        if matched:
    ...
    for index, (mz, intensity) in enumerate(mirror.peak_b):
        ...
        marks.append(
            f'<line x1="{x:.2f}" y1="{mid}" x2="{x:.2f}" y2="{y:.2f}" stroke="{color}" stroke-width="{stroke}"><title>Peak 2 m/z {mz:.5f}; intensity {intensity:.6g}; {"matched" if matched else "unmatched"}</title></line>'
        )
        if matched:
            marks.append(
                f'<path d="M {x:.2f} {y + 3.5:.2f} L {x - 3.5:.2f} {y - 2.5:.2f} L {x + 3.5:.2f} {y - 2.5:.2f} Z" fill="#d97706"/>'
            )
    for tick in range(7):
        mz = mirror.min_mz + (mirror.max_mz - mirror.min_mz) * tick / 6
        x = scale_x(mz)
        marks.append(
            f'<line x1="{x:.1f}" y1="{bottom}" x2="{x:.1f}" y2="{bottom + 5}" stroke="#8792a3"/><text x="{x:.1f}" y="{bottom + 20}" text-anchor="middle" class="mono" font-size="10" fill="#667085">{mz:.1f}</text>'
        )
    marks.append(
        f'<text x="{right}" y="350" text-anchor="end" font-size="10" fill="#667085">matched: thick + circle/triangle · unmatched: thin grey</text>'
    )
    return "".join(marks)


def _peak_card_svg(x, y, title, prefix, row, color):
    """绘制一侧RT窗口、apex、扫描数和fragment过滤摘要。"""
...
def _dia_coverage_svg(target_mz, windows):
    ...
    for index, (lower, upper) in enumerate(zip(lowers, uppers, strict=True)):
        marks.append(
            f'<rect x="{sx(lower):.2f}" y="741" width="{max(1.0, sx(upper) - sx(lower)):.2f}" height="18" fill="{"#dce9fb" if index % 2 == 0 else "#eef3fb"}" stroke="#8aaee3" stroke-width="0.6"/>'
        )
    if target is not None:
        x = sx(target)
        label = "inside acquired window" if covered else "outside acquired windows"
        marks.append(
            f'<line x1="{x:.2f}" y1="736" x2="{x:.2f}" y2="764" stroke="#d97706" stroke-width="2"/><text x="{min(max(x + 5, 75), 1040):.2f}" y="778" class="mono" font-size="10" fill="#8a4b08">target {target:.4f} · {label}</text>'
        )
    marks.append(
        f'<text x="1190" y="732" text-anchor="end" class="mono" font-size="10" fill="#667085">{min(lowers):.1f}–{max(uppers):.1f} m/z · n={len(windows)}</text>'
    )
    return "".join(marks)


def _number(value):
    """把SVG输入转换为有限浮点数；缺失或非法时返回``None``。"""
...
def _fmt(value, digits=3):
    """按指定小数位格式化SVG数值；缺失时显示破折号。"""

    number = _number(value)
    if number is None:
        return "—"
    return str(round(number)) if digits == 0 else f"{number:.{digits}f}"
...
def _observed_summary(context: dict, key: str) -> str:
    """将一个raw属性的多个观测值及计数压缩成短文本。"""

    values = (context.get("observed_values") or {}).get(key) or []
    if values:
        return "/".join(f"{item.get('value', '—')} (n={item.get('count', '—')})" for item in values)
    return str(context.get(key, "—"))


def _comparison_label(row: dict) -> str:
    """在shadow比较结果存在时追加参数变化提示。"""
