...
def _panel(
    ...
    *,
    ...
    sx = lambda value: left + (value - x_min) / x_span * plot_width
    ...
    for fraction in (0.25, 0.5, 0.75):
        ...
        parts.append(f'<line x1="{left}" y1="{y:.1f}" x2="{left+plot_width}" y2="{y:.1f}" stroke="#edf0f4"/>')
    if threshold is not None and transform(float(threshold)) <= maximum:
        y = sy(float(threshold))
        parts.append(f'<line x1="{left}" y1="{y:.1f}" x2="{left+plot_width}" y2="{y:.1f}" stroke="#bd3d45" stroke-dasharray="5 5"/><text x="{left+plot_width-4}" y="{y-4:.1f}" text-anchor="end" font-size="10" fill="#bd3d45">height floor {_compact(threshold)}</text>')
    selected_x = [x_values[i] for i in indices]
    parts.append(_polyline(selected_x, [raw[i] for i in indices], sx, sy, "#a8b2c1", 1.0, plot_width))
    parts.append(_polyline(selected_x, [baseline[i] for i in indices], sx, sy, "#246bce", 2.0, plot_width))
    parts.append(_polyline(selected_x, [experimental[i] for i in indices], sx, sy, "#d97706", 1.5, plot_width, dash="5 3"))
    for index, rt in enumerate(manual_rts):
        if x_min <= rt <= x_max:
            x = sx(rt)
            label_y = top + 13 + (index % 2) * 12
            parts.append(f'<line x1="{x:.1f}" y1="{top}" x2="{x:.1f}" y2="{top+plot_height}" stroke="#172033" stroke-dasharray="3 4"/><text x="{x+3:.1f}" y="{label_y}" font-size="10" fill="#172033">supplied {rt:.3f}</text>')
    for peak in baseline_peaks:
        rt = peak.rt_sec / 60
        if x_min <= rt <= x_max:
            parts.append(f'<circle cx="{sx(rt):.1f}" cy="{sy(peak.intensity):.1f}" r="4" fill="#246bce" stroke="#fff"/>')
    for peak in experimental_peaks:
        rt = peak.rt_sec / 60
        if x_min <= rt <= x_max:
            x, y = sx(rt), sy(peak.intensity)
            parts.append(f'<path d="M {x:.1f} {y-5:.1f} L {x-5:.1f} {y+4:.1f} L {x+5:.1f} {y+4:.1f} Z" fill="#d97706" stroke="#fff"/>')
    for candidate in review_candidates:
        rt = candidate.rt_sec / 60
        if x_min <= rt <= x_max:
            x, y = sx(rt), sy(candidate.intensity)
            parts.append(f'<rect x="{x-3.5:.1f}" y="{y-3.5:.1f}" width="7" height="7" fill="none" stroke="#7c3aed" stroke-width="1.4"><title>high-recall candidate RT {rt:.4f}; prominence ratio {candidate.prominence_ratio}; support {candidate.support_scans}</title></rect>')
    tick_count = 6 if not compact else 4
    for index in range(tick_count):
        value = x_min + x_span * index / max(tick_count - 1, 1)
        x = sx(value)
        parts.append(f'<text x="{x:.1f}" y="{top+plot_height+17}" text-anchor="middle" class="mono" font-size="10" fill="#667085">{value:.1f}</text>')
    parts.append(f'<text x="{left-8}" y="{top+11}" text-anchor="end" class="mono" font-size="9" fill="#667085">{_compact(maximum if not log_scale else (10**maximum-1))}</text>')
    return "".join(parts)
...
def _downsample_envelope(x_values, y_values, max_points):
    ...
    for bin_index in range(bins):
        ...
        chunk = list(zip(x_values[first:last], y_values[first:last]))
...
def _legend(metadata):
    ...
    experimental = html.escape(str(metadata.get("experimental_label", "experimental")).split(":", 1)[0])
    return ('<g transform="translate(510 226)"><line x1="0" y1="0" x2="28" y2="0" stroke="#a8b2c1"/><text x="34" y="4" font-size="11" fill="#667085">raw</text>'
